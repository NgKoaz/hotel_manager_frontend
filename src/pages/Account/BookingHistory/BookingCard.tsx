...
const BookingCard: React.FC<BookingCardProps> = ({ booking, bookingRes, loadBookings }) => {
    ...
    const paymentTransactions = bookingRes.paymentTransactions;

    const paymentStatusId = payment?.paymentStatusId;
    const bookingStatusId = booking?.bookingStatusId
    if (!paymentStatusId || !bookingStatusId) return;

    const isPayAllButton = 
        (bookingStatusId == BookingStatusEnum.PENDING || 
        bookingStatusId == BookingStatusEnum.CONFIRMED ||
        bookingStatusId == BookingStatusEnum.CHECKED_IN) &&
    ...
    const onPayAll = (bookingId: number) => {
        toast.error("Chưa tích hợp thanh toán online!");
    }

    const onDeposit = (bookingId: number) => {
        toast.error("Chưa tích hợp thanh toán online!");
    }

    const onCancel = (bookingId: number) => {
        myApi.cancelBooking(bookingId).then((data) => {
        ...
        <Card className="mb-6">
            <CardHeader>
                <CardTitle className="flex justify-between items-center">
                    ...
                    {getStatusBadge(booking.bookingStatusId)}

                </CardTitle>
                <p className="text-sm text-gray-500">
                
                    {formatDateTime(new Date(booking.checkinDate))} →{" "}
                    {formatDateTime(new Date(booking.checkoutDate))}
                </p>
            </CardHeader>
            ...
            <CardContent className="space-y-4">
                ...
                <div>
                    ...
                    {bookingRes && bookingRooms.filter(ba => ba.bookingId == booking.id).map((br) => {
                ...
                {booking &&
                    (
                        <div className="flex justify-end space-x-2 mt-4">
                            {isPayAllButton && (
                                <Button className="bg-blue-600 hover:bg-blue-500" onClick={() => onPayAll(booking.id)}>
                                    Trả hết
                                </Button>
                            )}
                            {isDepositButton && (
                                <Button className="bg-gray-950 hover:bg-gray-700" onClick={() => onDeposit(booking.id)}>
                                    Đặt cọc
                                </Button>
                            )}
                            {isCancelButton && (
